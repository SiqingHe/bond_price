from data_torch import metricDataset,metricCacheDataset,dataIter,DataIterPND,DataIterCache
...
def siameseTrain(input_dim,train_path,batch_size,save_path,margin = 0.5,epochs = 10):
    ...
    model_save = dtUtils.increment_path(str(Path(current_path).joinpath("model/torch").joinpath(today).joinpath("res")))
    ...
    for epoch in range(epochs):
        for i, data in enumerate(dataloader, 0):
            ...
            data1_x = data1[:,0:-1].reshape(data1.shape[0],1,input_dim).float().cuda()
            data2_x = data2[:,0:-1].reshape(data1.shape[0],1,input_dim).float().cuda()
            ...
            if i%20000 == 0 :
                save_except = str(Path(model_save).joinpath(Path(save_path).stem+"_{}_{}.pth".format(epoch,i)))
    ...
    # 保存训练好的Siamese网络模型
    torch.save(net.state_dict(), str(Path(model_save).joinpath(Path(save_path).name)))
...
if __name__ == "__main__":
    ...
    train_path =  r"D:\python_code\LSTM-master\bond_price\dealed_dir\combine0818to0818\data0930_nostd.pkl"
    batch_size = 200
    # 10000
    save_path = r"D:\python_code\LSTM-master\bond_price\model\torch\torch_resnet_metric1016.pth"
    # try:
